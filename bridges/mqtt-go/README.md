...
| `mpubsub` | `encryption.replay_window` | — | Optional Go duration (e.g. `30s`). When set to a positive value, outbound encrypted publishes are stamped with the bridge clock + a random nonce, and inbound encrypted packets that are stale or repeat a recently-seen nonce are dropped. `0s` (or omitting it) disables the check. Mirrors the ESPHome devices' `mpubsub.encryption.replay_window`; a positive window requires `encryption.key`. |
